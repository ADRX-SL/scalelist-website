<?php
/**
 * Plugin Name: CEO Archive – Public View (MU, Safe Names)
 * Description: Shortcode [ceo_archive] — grilla plana, búsqueda por título (AJAX) + paginación por URL (/page/2/), 96 por página. Mismo diseño; sin colisiones de nombres.
 * Version: 1.0.5
 * Author: Scalelist
 */
...
/* =========================================================
 * Helpers propios (prefijo ca_ceo_) — SIN colisionar nombres
 * ========================================================= */

/** Auto-detect CPT actual sin pisar ca_tc_defaults() */
function ca_ceo_autodefaults() {
    $current_post_type = '';
    ...
    return array(
        'cpts' => $current_post_type ?: 'ceo',
    );
}

function ca_ceo_display_name($post_id) {
    $name = '';
...
function ca_ceo_initial_letter($str) {
    $s = trim((string)$str);
    if ($s === '') return '#';
...
function ca_ceo_get_logo($post_id, $size = 'medium') {
    if (function_exists('get_field')) {
        foreach (array('profile_image_url') as $field) {
...
function ca_ceo_render_card_internal($pid) {
    $name  = ca_ceo_display_name($pid);
    $perma = get_permalink($pid);
    $logo  = ca_ceo_get_logo($pid);
    ob_start(); ?>
    <a class="ca-card" href="<?php echo esc_url($perma); ?>">
        <div class="ca-card-media">
            ...
            <?php else: ?>
                <div class="ca-avatar" aria-hidden="true"><?php echo esc_html(ca_ceo_initial_letter($name)); ?></div>
            <?php endif; ?>
        </div>
...
function ca_ceo_render_card($pid) {
    return ca_ceo_render_card_internal($pid);
}

/** Filtro title-only propio: no colisiona con otros */
function ca_ceo_title_only_search($search, $wp_query){
...
add_filter('posts_search', 'ca_ceo_title_only_search', 10, 2);

/** Paginación por URL conservando ca_s
 *  ➕ Acepta $base_override para corregir los links cuando se renderiza vía AJAX.
 */
function ca_ceo_render_pagination($current, $total, $search, $base_override = ''){
    if ($total <= 1) return '';
    if (!empty($base_override)) {
        // $base_override viene como path del front (p.ej. "/ceo/"), sin /page/N/
 ...
 */
function ca_ceo_render_results($cpts, $ppp, $search, $paged, $base_override = ''){
    // Conteo
    ...
    echo '<div class="ca-heading">';
      echo '<div class="ca-subtitle">Showing ' . intval($total_all) . ' Professionals</div>';
      if ($total_pages > 1) {
    ...
    if ($q->have_posts()){
        ...
        foreach ($q->posts as $pid){
            echo ca_ceo_render_card($pid);
        }
        echo '</div>';
        // 🧩 Usa base_override cuando hay AJAX; en render inicial queda vacío
        echo ca_ceo_render_pagination($paged, (int)$q->max_num_pages, (string)$search, $base_override);
    } else {
        echo '<div class="ca-empty">No professionals match your filters.</div>';
    }
 ...
 * Shortcode principal
 * ========================================================= */
function ca_ceo_shortcode($atts){
    // Preferimos los defaults del plugin original si existen
    $defaults = function_exists('ca_tc_defaults') ? ca_tc_defaults() : ca_ceo_autodefaults();
    ...
    <style>
      ...
      #ceo-loader{display:none;place-items:center;min-height:80px;margin:10px 0}
      #ceo-loader .spinner{width:36px;height:36px;border:3px solid #cbd5e1;border-top-color:#2563eb;border-radius:50%;animation:ca-spin .8s linear infinite}
      @keyframes ca-spin{to{transform:rotate(360deg)}}
      .ceo-results.is-loading{opacity:.4;pointer-events:none;transition:opacity .2s}
    ...
    $results_html = ca_ceo_render_results($cpts, $ppp, $search, $paged);

    ob_start(); ?>
    <div class="ca-archive ceo-archive"
         data-enable-ajax="<?php echo (int)$a['enable_ajax']; ?>"
         data-cpts="<?php echo esc_attr(implode(',', $cpts)); ?>"
         data-ppp="<?php echo (int)$ppp; ?>">
      <?php echo $styles; ?>

      <form id="ceo-controls" class="ca-controls" method="get">
        <div class="row">
          <div>
            <input id="ceo_s" class="ca-input" type="search" name="ca_s"
      ...
      <div id="ceo-loader" aria-hidden="true"><div class="spinner"></div></div>

      <div class="ca-results ceo-results" id="ceo-results">
    ...
    if (!is_admin() && (int)$a['enable_ajax']) {
        add_action('wp_footer', 'ca_ceo_footer_script', 100);
    }

    return ob_get_clean();
}
add_shortcode('ceo_archive', 'ca_ceo_shortcode');

/* =========================================================
 * Footer JS (una sola vez)
 * ========================================================= */
function ca_ceo_footer_script(){
    static $done = false; if ($done) return; $done = true;
    $nonce = wp_create_nonce('ca_ceo_nonce');
    $ajax  = admin_url('admin-ajax.php'); ?>
<script>
(function(){
  var blocks = document.querySelectorAll('.ceo-archive[data-enable-ajax="1"]');
  if(!blocks.length) return;

  blocks.forEach(function(root){
    var form   = root.querySelector('#ceo-controls');
    var inputS = root.querySelector('#ceo_s');
    var results= root.querySelector('#ceo-results');
    var loader = root.querySelector('#ceo-loader');
    ...
    function fetchPublic(params, cb){
      ...
      data.append('action','ca_ceo_fetch');
      ...
      fetch('<?php echo esc_url($ajax); ?>', { method:'POST', body:data, credentials:'same-origin' })
        ...
        .catch(function(e){
          hideL();
          console.error('CEO Archive AJAX error:', e);
 ...
 * ========================================================= */
add_action('wp_ajax_ca_ceo_fetch', 'ca_ceo_fetch');
add_action('wp_ajax_nopriv_ca_ceo_fetch', 'ca_ceo_fetch');

function ca_ceo_fetch(){
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field($_POST['nonce']), 'ca_ceo_nonce')) {
    ...
    if (empty($cpts)) {
        $defs = function_exists('ca_tc_defaults') ? ca_tc_defaults() : ca_ceo_autodefaults();
        $cpts = array($defs['cpts'] ?: 'ceo');
    }
    ...
    $html = ca_ceo_render_results($cpts, $ppp, $search, $paged, $base_url);
